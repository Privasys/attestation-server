# Installing the Attestation Server on OVH Cloud

This guide walks through deploying the Privasys Attestation Server on an
OVH Cloud instance with Intel DCAP support, fronted by Caddy for automatic HTTPS.

---

## 1. Create the instance

In the OVH Cloud Manager, create a Public Cloud instance.
The server only verifies quotes — it does **not** need to run inside a TEE itself.
A small general-purpose instance is sufficient (e.g. `b3-8`, 2 vCPUs / 8 GB RAM,
Ubuntu 24.04). Choose a region close to your workloads (e.g. `GRA11`, `SBG5`).

Make sure the instance has a public IP and that TCP ports **80** and **443** are
open in the security group (required by Caddy for automatic HTTPS).
...
[Service]
Type=simple
WorkingDirectory=/home/ubuntu/attestation-server
Environment=JWT_SIGNING_KEY_FILE=/home/ubuntu/attestation-server/server-jwt.key
ExecStart=/home/ubuntu/attestation-server/dist/attestation-server
Restart=on-failure
RestartSec=5
...
Edit `/etc/caddy/Caddyfile` (replace the domain with your own):
...
```bash
curl -X POST https://attestation.example.com/api/verify \
